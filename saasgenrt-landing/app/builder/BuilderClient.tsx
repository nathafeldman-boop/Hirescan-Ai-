...
const IDEA_ACCENT = ['#c8442a', '#0ea5e9', '#10b981', '#f59e0b', '#ec4899', '#6366f1', '#14b8a6', '#f43f5e', '#a855f7', '#22d3ee']
...
function competitionLabel(score: number) {
  if (score <= 3) return { label: 'Faible', color: '#23643e' }
  if (score <= 6) return { label: 'Moyen', color: '#b45309' }
  return { label: 'Élevé', color: '#b91c1c' }
}

function complexityStyle(c: string) {
  if (c === 'Faible') return { color: 'var(--ok)', bg: 'var(--ok-wash)', border: 'oklch(0.52 0.135 152 / 0.22)' }
  if (c === 'Moyen') return { color: '#b45309', bg: 'oklch(0.70 0.13 70 / 0.12)', border: 'oklch(0.70 0.13 70 / 0.28)' }
  return { color: 'var(--error)', bg: 'oklch(0.55 0.20 25 / 0.08)', border: 'oklch(0.55 0.20 25 / 0.22)' }
}
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        background: disabled ? 'var(--bg-2)' : 'var(--signal)',
        color: disabled ? 'var(--muted)' : 'white',
        boxShadow: disabled ? 'none' : isAnalyze ? 'var(--shadow-md)' : 'var(--shadow-sm)',
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        ...
        el.style.color = 'var(--ink-soft)'; el.style.borderColor = 'var(--border-strong)'
      }}
      onMouseLeave={(e) => {
        const el = e.currentTarget as HTMLElement
...
function StepBadge({ n, tone = 'purple' }: { n: number | string; tone?: 'purple' | 'green' }) {
  ...
  const c = tone === 'green'
    ? { bg: 'var(--ok-wash)', border: 'oklch(0.52 0.135 152 / 0.28)', dot: 'var(--ok)', text: 'var(--ok)' }
    : { bg: 'var(--signal-wash)', border: 'oklch(0.505 0.19 30 / 0.22)', dot: 'var(--signal)', text: 'var(--signal-ink)' }
  return (
    <motion.div
  ...
  return (
    <>
      <motion.h2
        ...
        style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(23px, 3.1vw, 33px)', fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.03em', lineHeight: 1.15, marginBottom: 11 }}
      >
        {pre}
        {accent && (
          <>
      ...
      <motion.p
        ...
        style={{ fontSize: '14px', lineHeight: 1.68, color: 'var(--muted)', marginBottom: 26 }}
      >
        {subtitle}
      </motion.p>
    </>
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 9 }}>
        <span style={{ fontSize: '11px', fontWeight: 700, color: 'var(--muted)', letterSpacing: '0.07em' }}>
          ÉTAPE {current} / {total}
        </span>
      ...
      <div style={{ height: 4, background: 'var(--bg-2)', borderRadius: 99, overflow: 'hidden' }}>
        <motion.div
          ...
          style={{ height: '100%', borderRadius: 99, background: 'var(--signal)' }}
        />
      </div>
    </div>
  )
...
function SelectionCard({ selected, onClick, icon, label, desc, accent = '#c8442a', index = 0, big }: {
  ...
  return (
    <motion.button
      ...
      style={{
        ...
        background: selected ? `${accent}1f` : 'var(--surface)',
        border: `1.5px solid ${selected ? accent : 'var(--border)'}`,
        boxShadow: selected ? `0 0 0 1px ${accent}30, var(--shadow-md)` : 'var(--shadow-sm)',
    ...
    >
      ...
      <div style={{ fontSize: big ? '17px' : '13.5px', fontWeight: 700, marginBottom: desc ? 5 : 0, color: 'var(--ink)' }}>
        {label}
      </div>
      {desc && <div style={{ fontSize: big ? '13px' : '11.5px', color: 'var(--muted)', lineHeight: 1.5 }}>{desc}</div>}
    </motion.button>
  )
}
  ...
  return (
    <motion.div animate={reduce ? undefined : { y: [0, -7, 0] }} transition={{ duration: 4.5, repeat: Infinity, ease: 'easeInOut' }}>
      <svg width="280" height="338" viewBox="0 0 280 338" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect width="280" height="338" rx="20" fill="var(--surface)" />
        <rect width="280" height="338" rx="20" stroke="var(--border)" strokeWidth="1" />
        <rect x="16" y="16" width="248" height="198" rx="10" fill="var(--bg-2)" stroke="var(--border)" strokeWidth="1" />
        <line x1="180" y1="82" x2="180" y2="116" stroke="rgba(200,68,42,0.32)" strokeWidth="1.2" strokeDasharray="4,3" />
        <line x1="202" y1="66" x2="210" y2="116" stroke="rgba(46,125,80,0.25)" strokeWidth="1.2" strokeDasharray="4,3" />
        <circle cx="180" cy="99" r="2.5" fill="rgba(200,68,42,0.55)" />
        <g transform="rotate(-3,64,62)"><rect x="32" y="38" width="64" height="48" rx="5" fill="rgba(180,83,9,0.14)" stroke="rgba(180,83,9,0.30)" strokeWidth="1" /><circle cx="64" cy="41" r="3" fill="rgba(180,83,9,0.48)" /><text x="64" y="66" textAnchor="middle" fill="rgba(146,64,14,0.92)" fontSize="8.5" fontWeight="600">Douleur</text><text x="64" y="78" textAnchor="middle" fill="rgba(146,64,14,0.92)" fontSize="8.5" fontWeight="600">Client</text></g>
        <g transform="rotate(2,148,57)"><rect x="116" y="32" width="64" height="50" rx="5" fill="rgba(200,68,42,0.16)" stroke="rgba(200,68,42,0.40)" strokeWidth="1" /><circle cx="148" cy="35" r="3" fill="rgba(200,68,42,0.62)" /><text x="148" y="60" textAnchor="middle" fill="rgba(160,50,30,0.95)" fontSize="8.5" fontWeight="600">Opportunité</text><text x="148" y="73" textAnchor="middle" fill="rgba(160,50,30,0.95)" fontSize="8.5" fontWeight="600">Marché</text></g>
        <g transform="rotate(-2,228,62)"><rect x="198" y="40" width="60" height="44" rx="5" fill="rgba(46,125,80,0.14)" stroke="rgba(46,125,80,0.28)" strokeWidth="1" /><circle cx="228" cy="43" r="3" fill="rgba(46,125,80,0.55)" /><text x="228" y="65" textAnchor="middle" fill="rgba(35,100,62,0.95)" fontSize="8.5" fontWeight="600">MRR</text><text x="228" y="77" textAnchor="middle" fill="rgba(35,100,62,0.95)" fontSize="8.5" fontWeight="600">Potentiel</text></g>
        <g transform="rotate(3,66,143)"><rect x="36" y="120" width="60" height="46" rx="5" fill="rgba(14,165,233,0.14)" stroke="rgba(14,165,233,0.28)" strokeWidth="1" /><circle cx="66" cy="123" r="3" fill="rgba(14,165,233,0.48)" /><text x="66" y="145" textAnchor="middle" fill="rgba(2,132,199,0.92)" fontSize="8.5" fontWeight="600">Solution</text><text x="66" y="157" textAnchor="middle" fill="rgba(2,132,199,0.92)" fontSize="8.5" fontWeight="600">Concept</text></g>
        <g transform="rotate(-2,180,139)"><rect x="148" y="116" width="64" height="48" rx="5" fill="rgba(219,39,119,0.13)" stroke="rgba(219,39,119,0.28)" strokeWidth="1" /><circle cx="180" cy="119" r="3" fill="rgba(219,39,119,0.48)" /><text x="180" y="140" textAnchor="middle" fill="rgba(190,24,93,0.92)" fontSize="8.5" fontWeight="600">Audience</text><text x="180" y="153" textAnchor="middle" fill="rgba(190,24,93,0.92)" fontSize="8.5" fontWeight="600">Cible</text></g>
        <line x1="16" y1="230" x2="264" y2="230" stroke="var(--border)" strokeWidth="1" />
        <rect x="18" y="246" width="78" height="74" rx="7" fill="var(--bg-2)" stroke="var(--border)" strokeWidth="1" /><rect x="18" y="246" width="9" height="74" rx="3" fill="rgba(200,68,42,0.22)" /><line x1="35" y1="264" x2="88" y2="264" stroke="var(--border-strong)" strokeWidth="1" /><line x1="35" y1="275" x2="88" y2="275" stroke="var(--border)" strokeWidth="1" /><line x1="35" y1="286" x2="88" y2="286" stroke="var(--border)" strokeWidth="1" />
        <rect x="114" y="258" width="36" height="44" rx="6" fill="var(--bg-2)" stroke="var(--border)" strokeWidth="1" /><rect x="118" y="262" width="28" height="12" rx="3" fill="rgba(200,68,42,0.20)" /><path d="M150 272 Q164 272 164 282 Q164 292 150 292" stroke="var(--border-strong)" strokeWidth="1.5" fill="none" strokeLinecap="round" />
        <rect x="176" y="248" width="86" height="72" rx="9" fill="var(--bg-2)" stroke="var(--border)" strokeWidth="1" /><rect x="182" y="254" width="74" height="54" rx="5" fill="var(--bg)" />
        <rect x="192" y="294" width="9" height="10" rx="2" fill="rgba(200,68,42,0.45)" /><rect x="204" y="287" width="9" height="17" rx="2" fill="rgba(200,68,42,0.55)" /><rect x="216" y="279" width="9" height="25" rx="2" fill="rgba(200,68,42,0.70)" /><rect x="228" y="272" width="9" height="32" rx="2" fill="rgba(46,125,80,0.65)" /><rect x="240" y="266" width="9" height="38" rx="2" fill="rgba(46,125,80,0.85)" /><text x="219" y="267" textAnchor="middle" fill="rgba(46,125,80,0.75)" fontSize="6.5" fontWeight="700">MRR ↑</text>
      </svg>
    </motion.div>
  )
}
...
function AIHelperCard({ onUseExample }: { onUseExample: (text: string) => void }) {
  return (
    <div style={{ borderRadius: 16, padding: '18px 16px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 14 }}>
        <div style={{ width: 30, height: 30, borderRadius: 9, flexShrink: 0, background: 'var(--signal-wash)', border: '1px solid oklch(0.505 0.19 30 / 0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' }}>💡</div>
        <span style={{ fontSize: '13.5px', fontWeight: 600, color: 'var(--ink)' }}>Need inspiration?</span>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {AI_EXAMPLES.map((ex, i) => (
          <button key={i} onClick={() => onUseExample(ex)}
            style={{ padding: '9px 12px', borderRadius: 10, textAlign: 'left', cursor: 'pointer', background: 'var(--bg)', border: '1px solid var(--border)', fontSize: '12px', color: 'var(--ink-soft)', lineHeight: 1.5, transition: 'all 0.15s', fontFamily: 'inherit' }}
            onMouseEnter={(e) => { const el = e.currentTarget as HTMLElement; el.style.background = 'var(--signal-wash)'; el.style.borderColor = 'oklch(0.505 0.19 30 / 0.22)'; el.style.color = 'var(--ink)' }}
            onMouseLeave={(e) => { const el = e.currentTarget as HTMLElement; el.style.background = 'var(--bg)'; el.style.borderColor = 'var(--border)'; el.style.color = 'var(--ink-soft)' }}>
            &quot;{ex}&quot;
          </button>
        ))}
      </div>
  ...
  return (
    <motion.div animate={reduce ? undefined : { y: [0, -6, 0] }} transition={{ duration: 4.2, repeat: Infinity, ease: 'easeInOut' }}>
      <svg width="260" height="320" viewBox="0 0 260 320" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect width="260" height="320" rx="20" fill="var(--surface)" /><rect width="260" height="320" rx="20" stroke="var(--border)" strokeWidth="1" />
        <ellipse cx="130" cy="45" rx="90" ry="50" fill="rgba(200,68,42,0.07)" />
        <ellipse cx="130" cy="162" rx="62" ry="54" fill="var(--bg-2)" stroke="var(--border)" strokeWidth="1" />
        <circle cx="130" cy="78" r="17" fill="rgba(200,68,42,0.16)" stroke="rgba(200,68,42,0.40)" strokeWidth="1" /><circle cx="130" cy="74" r="7" fill="rgba(200,68,42,0.55)" />
        <circle cx="40" cy="162" r="16" fill="rgba(14,165,233,0.14)" stroke="rgba(14,165,233,0.32)" strokeWidth="1" /><circle cx="40" cy="158" r="7" fill="rgba(2,132,199,0.45)" />
        <circle cx="220" cy="162" r="16" fill="rgba(46,125,80,0.12)" stroke="rgba(46,125,80,0.28)" strokeWidth="1" /><circle cx="220" cy="158" r="7" fill="rgba(46,125,80,0.40)" />
        <circle cx="130" cy="246" r="16" fill="rgba(219,39,119,0.12)" stroke="rgba(219,39,119,0.28)" strokeWidth="1" /><circle cx="130" cy="242" r="7" fill="rgba(219,39,119,0.40)" />
        <rect x="150" y="54" width="62" height="24" rx="8" fill="rgba(200,68,42,0.16)" stroke="rgba(200,68,42,0.30)" strokeWidth="1" /><text x="181" y="70" textAnchor="middle" fill="rgba(160,50,30,0.95)" fontSize="7.5" fontWeight="600">Problème ?</text>
        <rect x="14" y="138" width="60" height="22" rx="7" fill="rgba(14,165,233,0.14)" stroke="rgba(14,165,233,0.26)" strokeWidth="1" /><text x="44" y="153" textAnchor="middle" fill="rgba(2,132,199,0.92)" fontSize="7.5" fontWeight="600">Chaque sem. !</text>
        <rect x="186" y="138" width="60" height="22" rx="7" fill="rgba(46,125,80,0.10)" stroke="rgba(46,125,80,0.22)" strokeWidth="1" /><text x="216" y="153" textAnchor="middle" fill="rgba(35,100,62,0.95)" fontSize="7.5" fontWeight="600">Grand marché !</text>
        <rect x="16" y="288" width="228" height="22" rx="7" fill="var(--bg-2)" stroke="var(--border)" strokeWidth="1" /><circle cx="32" cy="299" r="4" fill="rgba(46,125,80,0.55)" /><text x="42" y="303" fontSize="7.5" fill="var(--muted)" fontWeight="500">4 personas identifiés</text>
      </svg>
    </motion.div>
  )
}
...
function BifurcationIllustration() {
  ...
  return (
    <motion.div animate={reduce ? undefined : { y: [0, -6, 0] }} transition={{ duration: 4.4, repeat: Infinity, ease: 'easeInOut' }}>
      <svg width="260" height="320" viewBox="0 0 260 320" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect width="260" height="320" rx="20" fill="var(--surface)" /><rect width="260" height="320" rx="20" stroke="var(--border)" strokeWidth="1" />
        <ellipse cx="130" cy="270" rx="100" ry="44" fill="rgba(200,68,42,0.06)" />
        <circle cx="130" cy="258" r="9" fill="rgba(200,68,42,0.5)" stroke="rgba(200,68,42,0.7)" strokeWidth="1.5" />
        <path d="M130 250 C 120 200, 80 150, 64 96" stroke="rgba(200,68,42,0.55)" strokeWidth="2.5" fill="none" strokeLinecap="round" />
        <path d="M130 250 C 145 200, 190 160, 200 96" stroke="rgba(2,132,199,0.4)" strokeWidth="2" fill="none" strokeLinecap="round" strokeDasharray="5,5" />
        <circle cx="64" cy="74" r="22" fill="rgba(200,68,42,0.16)" stroke="rgba(200,68,42,0.45)" strokeWidth="1.5" />
        <path d="M64 62 L67 71 L76 71 L69 77 L72 86 L64 80 L56 86 L59 77 L52 71 L61 71 Z" fill="rgba(200,68,42,0.9)" />
        <text x="64" y="116" textAnchor="middle" fill="rgba(160,50,30,0.95)" fontSize="9" fontWeight="700">OBJECTIF CLAIR</text>
        <circle cx="200" cy="74" r="22" fill="rgba(2,132,199,0.10)" stroke="rgba(2,132,199,0.3)" strokeWidth="1.5" strokeDasharray="4,4" />
        <text x="200" y="80" textAnchor="middle" fill="rgba(2,132,199,0.85)" fontSize="18" fontWeight="700">?</text>
        <text x="200" y="116" textAnchor="middle" fill="rgba(2,132,199,0.75)" fontSize="9" fontWeight="700">ON EXPLORE</text>
        <circle cx="100" cy="180" r="2.5" fill="var(--border-strong)" /><circle cx="160" cy="190" r="2.5" fill="var(--border)" /><circle cx="130" cy="160" r="2" fill="rgba(200,68,42,0.4)" />
        <rect x="40" y="280" width="180" height="22" rx="7" fill="var(--bg-2)" stroke="var(--border)" strokeWidth="1" /><text x="130" y="294" textAnchor="middle" fontSize="8" fill="var(--muted)" fontWeight="600">Choisis ton chemin</text>
      </svg>
    </motion.div>
  )
}
...
function DiscoveryIllustration() {
  ...
  return (
    <motion.div animate={reduce ? undefined : { y: [0, -6, 0] }} transition={{ duration: 4.6, repeat: Infinity, ease: 'easeInOut' }}>
      <svg width="260" height="320" viewBox="0 0 260 320" fill="none" xmlns="http://www.w3.org/2000/svg">
        <rect width="260" height="320" rx="20" fill="var(--surface)" /><rect width="260" height="320" rx="20" stroke="var(--border)" strokeWidth="1" />
        <ellipse cx="130" cy="120" rx="95" ry="80" fill="rgba(200,68,42,0.05)" />
        <line x1="70" y1="80" x2="130" y2="118" stroke="var(--border-strong)" strokeWidth="1" /><line x1="190" y1="70" x2="130" y2="118" stroke="var(--border-strong)" strokeWidth="1" /><line x1="60" y1="160" x2="130" y2="118" stroke="rgba(200,68,42,0.3)" strokeWidth="1" /><line x1="200" y1="170" x2="130" y2="118" stroke="var(--border)" strokeWidth="1" />
        <circle cx="70" cy="80" r="5" fill="rgba(180,83,9,0.6)" /><circle cx="190" cy="70" r="5" fill="rgba(2,132,199,0.55)" /><circle cx="60" cy="160" r="5" fill="rgba(46,125,80,0.6)" /><circle cx="200" cy="170" r="5" fill="rgba(219,39,119,0.55)" />
        <circle cx="118" cy="108" r="34" fill="none" stroke="rgba(200,68,42,0.55)" strokeWidth="3" />
        <line x1="142" y1="132" x2="166" y2="156" stroke="rgba(200,68,42,0.55)" strokeWidth="4" strokeLinecap="round" />
        <text x="118" y="118" textAnchor="middle" fontSize="26">💡</text>
        <rect x="24" y="226" width="212" height="74" rx="12" fill="var(--bg-2)" stroke="var(--border)" strokeWidth="1" />
        <line x1="40" y1="246" x2="180" y2="246" stroke="var(--border-strong)" strokeWidth="2" strokeLinecap="round" /><line x1="40" y1="262" x2="210" y2="262" stroke="var(--border)" strokeWidth="2" strokeLinecap="round" /><line x1="40" y1="278" x2="150" y2="278" stroke="rgba(200,68,42,0.4)" strokeWidth="2" strokeLinecap="round" />
      </svg>
    </motion.div>
  )
}
  ...
  return (
    <motion.div animate={reduce ? undefined : { y: [0, -7, 0] }} transition={{ duration: 4.5, repeat: Infinity, ease: 'easeInOut' }}>
      <svg width="220" height="240" viewBox="0 0 220 240" fill="none" xmlns="http://www.w3.org/2000/svg">
        <ellipse cx="110" cy="120" rx="90" ry="90" fill="rgba(46,125,80,0.05)" />
        <rect x="55" y="40" width="110" height="150" rx="10" fill="var(--bg-2)" stroke="var(--border)" strokeWidth="1" />
        <path d="M55 50 L65 44 L75 50 L85 44 L95 50 L105 44 L115 50 L125 44 L135 50 L145 44 L155 50 L165 44 L165 40 L55 40 Z" fill="var(--bg-2)" />
        <line x1="70" y1="74" x2="150" y2="74" stroke="var(--border-strong)" strokeWidth="2" strokeLinecap="round" /><line x1="70" y1="90" x2="130" y2="90" stroke="var(--border)" strokeWidth="2" strokeLinecap="round" /><line x1="70" y1="106" x2="140" y2="106" stroke="var(--border)" strokeWidth="2" strokeLinecap="round" />
        <line x1="70" y1="132" x2="120" y2="132" stroke="rgba(46,125,80,0.5)" strokeWidth="2" strokeLinecap="round" /><text x="150" y="137" textAnchor="end" fill="rgba(35,100,62,0.95)" fontSize="13" fontWeight="800">€25</text>
        <circle cx="110" cy="170" r="14" fill="rgba(46,125,80,0.15)" stroke="rgba(46,125,80,0.5)" strokeWidth="1.5" /><path d="M104 170 L108 174 L116 165" stroke="rgba(46,125,80,0.95)" strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
        <motion.g animate={reduce ? undefined : { opacity: [0.3, 1, 0.3] }} transition={{ duration: 2, repeat: Infinity }}>
          <path d="M180 60 L183 68 L191 68 L184 73 L187 82 L180 77 L173 82 L176 73 L169 68 L177 68 Z" fill="rgba(180,83,9,0.7)" />
        </motion.g>
        <motion.circle cx="40" cy="100" r="3" fill="rgba(200,68,42,0.6)" animate={reduce ? undefined : { opacity: [0.3, 1, 0.3] }} transition={{ duration: 2.5, repeat: Infinity }} />
      </svg>
    </motion.div>
  )
}
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: 'center', marginBottom: 30 }}>
        <motion.div {...rise(0)}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 13px', borderRadius: 99, marginBottom: 20, background: 'var(--signal-wash)', border: '1px solid oklch(0.505 0.19 30 / 0.22)' }}>
          ...
          <span style={{ fontSize: '12px', color: 'var(--signal-ink)', fontWeight: 500, letterSpacing: '0.01em' }}>IA Product Builder</span>
        </motion.div>

        <motion.h1 {...rise(0.08)}
          style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(32px, 4.6vw, 46px)', fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.04, color: 'var(--ink)', marginBottom: 16, textWrap: 'balance' }}>
          {`Construisons ton prochain `}
          <span style={{ color: 'var(--signal)' }}>SaaS.</span>
        </motion.h1>

        <motion.p {...rise(0.16)}
          style={{ fontSize: '15px', lineHeight: 1.6, color: 'var(--muted)', maxWidth: 392, margin: '0 auto' }}>
      ...
      <motion.div {...rise(0.24)}
        style={{ borderRadius: 24, padding: 6, background: 'var(--bg-2)', border: '1px solid var(--border)', marginBottom: 26 }}>
        <div style={{ position: 'relative', borderRadius: 18, padding: '22px 22px 20px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
          {/* connecting line */}
          <motion.div aria-hidden
            initial={reduce ? false : { scaleY: 0 }} animate={{ scaleY: 1 }}
            transition={{ duration: 0.9, delay: 0.4, ease: EASE }}
            style={{ position: 'absolute', left: 40, top: 40, bottom: 42, width: 2, transformOrigin: 'top', borderRadius: 2, background: 'linear-gradient(180deg, var(--signal) 0%, var(--ok) 100%)', opacity: 0.55 }} />

          <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', gap: 18 }}>
            {JOURNEY.map(({ Icon, title, desc, payoff }, i) => {
              const tint = payoff ? 'var(--ok)' : 'var(--signal)'
              const tileBg = payoff ? 'var(--ok-wash)' : 'var(--signal-wash)'
              const tileBorder = payoff ? 'oklch(0.52 0.135 152 / 0.3)' : 'oklch(0.505 0.19 30 / 0.22)'
              return (
                <motion.div key={i}
                  initial={reduce ? false : { opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.45, delay: 0.42 + i * 0.09, ease: EASE }}
                  style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
                  <div style={{ width: 36, height: 36, flexShrink: 0, borderRadius: 11, background: tileBg, border: `1px solid ${tileBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: 'var(--shadow-sm)' }}>
                    <Icon style={{ width: 17, height: 17, color: tint }} strokeWidth={1.75} />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.01em', marginBottom: 2 }}>{title}</div>
                    <div style={{ fontSize: '12.5px', color: 'var(--muted)', lineHeight: 1.45 }}>{desc}</div>
                  </div>
                </motion.div>
              )
            })}
      ...
      <motion.button {...rise(0.34)}
        ...
        variants={{ rest: { scale: 1, boxShadow: 'var(--shadow-md)' }, hover: { scale: 1.02, boxShadow: 'var(--shadow-lg)' } }}
        animate="rest"
        style={{ width: '100%', padding: '13px 14px 13px 24px', borderRadius: 16, background: 'var(--signal)', color: 'white', fontWeight: 600, fontSize: '15px', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 18, fontFamily: 'inherit' }}>
        <span style={{ paddingLeft: 2 }}>Commencer</span>
        <motion.span variants={{ rest: { x: 0 }, hover: { x: 3 } }} transition={{ duration: 0.3, ease: EASE }}
          style={{ width: 30, height: 30, borderRadius: '50%', background: 'rgba(255,255,255,0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <motion.div {...rise(0.42)}
        ...
        {INTRO_TRUST.map(({ Icon, label }) => (
          <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <Icon style={{ width: 13, height: 13, color: 'var(--signal)', flexShrink: 0 }} strokeWidth={2} />
        ...
        <div style={{ flex: 1, minWidth: 300 }}>
          ...
          <motion.div
            ...
            style={{
              ...
              background: 'var(--bg-2)',
              border: `1px solid ${focused ? 'var(--signal)' : 'var(--border)'}`,
              boxShadow: focused ? '0 0 0 3px var(--signal-wash), var(--shadow-md)' : 'var(--shadow-sm)',
          ...
          >
            <div style={{
              ...
              border: `1px solid ${focused ? 'oklch(0.505 0.19 30 / 0.22)' : 'var(--border)'}`,
              ...
              <textarea
                ...
                style={{
                  ...
                  background: 'transparent', color: 'var(--ink)', fontSize: '14.5px', lineHeight: 1.72,
              ...
              <div style={{
                ...
                padding: '8px 18px 10px', borderTop: '1px solid var(--border)',
              }}>
                <motion.span
                  animate={{ color: charCount >= 30 ? 'var(--ok)' : 'var(--muted)' }}
                ...
                <span style={{ fontSize: '11px', color: charCount > 450 ? '#b45309' : 'var(--muted)' }}>
                  {charCount} / 500
                </span>
              </div>
            </div>
          ...
          <motion.div {...rise(0.3)} style={{ marginBottom: 28 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              ...
              <span style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 600, letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>
              ...
              {DOMAIN_CHIPS.map((chip, i) => (
                <motion.button
                  ...
                  style={{
                    ...
                    background: userData.domainChip === chip ? 'var(--signal-wash)' : 'var(--surface)',
                    border: `1px solid ${userData.domainChip === chip ? 'var(--signal)' : 'var(--border)'}`,
                    color: userData.domainChip === chip ? 'var(--signal-ink)' : 'var(--ink-soft)',
                    transition: 'background 0.15s, border-color 0.15s, color 0.15s',
                    boxShadow: userData.domainChip === chip ? 'var(--shadow-sm)' : 'none',
                  }}
                >
                  {chip}
                </motion.button>
        ...
        <div style={{ width: 280, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <motion.div
            ...
            style={{
              ...
              background: 'var(--bg-2)',
              border: '1px solid oklch(0.505 0.19 30 / 0.22)',
            }}
          >
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                <div style={{
                  ...
                  background: 'var(--signal-wash)', border: '1px solid oklch(0.505 0.19 30 / 0.22)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  <Sparkles style={{ width: 13, height: 13, color: 'var(--signal)' }} strokeWidth={2} />
                </div>
                <span style={{ fontSize: '12.5px', fontWeight: 600, color: 'var(--ink)' }}>Besoin d'inspiration ?</span>
              </div>
                ...
                {AI_EXAMPLES.map((ex, i) => (
                  <motion.button
                    ...
                    style={{
                      ...
                      fontSize: '11.5px', color: 'var(--ink-soft)', lineHeight: 1.55,
                      transition: 'background 0.14s, border-color 0.14s, color 0.14s',
                      fontFamily: 'inherit', display: 'flex', alignItems: 'flex-start', gap: 7,
                    }}
                    onMouseEnter={(e) => {
                      const el = e.currentTarget as HTMLElement
                      el.style.background = 'var(--signal-wash)'
                      el.style.borderColor = 'oklch(0.505 0.19 30 / 0.22)'
                  ...
                  >
                    <ArrowRight style={{ width: 10, height: 10, flexShrink: 0, marginTop: 2, color: 'var(--signal)' }} strokeWidth={2.5} />
                    <span>&quot;{ex}&quot;</span>
                  </motion.button>
                ))}
              </div>
          ...
          <div style={{ minHeight: 44, marginBottom: 16 }}>
            <AnimatePresence mode="wait">
              {value && (
                ...
                >
                  <div style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--signal)', flexShrink: 0 }} />
                  <span style={{ fontSize: '12px', color: 'var(--ink-soft)', lineHeight: 1.45 }}>
...
const KNOWS_CHOICES = [
  {
    ...
    accent: '#c8442a', accentFill: 'rgba(200,68,42,0.12)', accentBorder: 'rgba(200,68,42,0.36)',
  },
  {
    value: 'no', emoji: '🧭',
            ...
            return (
              <motion.button
                ...
                style={{
                  ...
                  background: sel ? c.accentFill : 'var(--surface)',
                  border: `1.5px solid ${sel ? c.accent : 'var(--border)'}`,
                  boxShadow: sel
                    ? `0 0 0 1px ${c.accentBorder}, var(--shadow-md)`
                    : 'var(--shadow-sm)',
              ...
              >
                ...
                <div style={{ fontFamily: 'var(--font-display)', fontSize: '16px', fontWeight: 700, color: 'var(--ink)', letterSpacing: '-0.02em', marginBottom: 8 }}>
                  {c.label}
                </div>
                <div style={{ fontSize: '12.5px', color: 'var(--muted)', lineHeight: 1.62, marginBottom: 18 }}>
                  {c.desc}
                </div>
                <div style={{
                  display: 'inline-flex', alignItems: 'center', gap: 5,
                  padding: '4px 10px', borderRadius: 99,
                  background: sel ? `${c.accent}22` : 'var(--bg-2)',
                  border: `1px solid ${sel ? c.accentBorder : 'var(--border)'}`,
                  transition: 'background 0.2s, border-color 0.2s',
                }}>
                  <span style={{ fontSize: '11px', fontWeight: 600, color: sel ? c.accent : 'var(--muted)', letterSpacing: '0.02em' }}>
                    {c.tag}
                  </span>
                </div>
              </motion.button>
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{
        ...
        background: 'var(--bg-2)',
        border: `1px solid ${focused ? 'var(--signal)' : 'var(--border)'}`,
        boxShadow: focused ? '0 0 0 3px var(--signal-wash)' : 'none',
        ...
        <textarea
          ...
          style={{
            ...
            background: 'var(--surface)',
            border: `1px solid ${focused ? 'oklch(0.505 0.19 30 / 0.22)' : 'var(--border)'}`,
            color: 'var(--ink)', fontSize: '14px', lineHeight: 1.65,
...
function ProblemListStep({ userData, onSelect, onNext, onBack }: {
  ...
  const freqColor = (f: string) => f === 'Quotidien' ? '#b91c1c' : f === 'Hebdomadaire' ? '#b45309' : '#23643e'
      ...
      <div style={{ textAlign: 'center', padding: '52px 0' }}>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 30, position: 'relative' }}>
          ...
          {!reduce && (
            <motion.div
              ...
              style={{ position: 'absolute', width: 88, height: 88, borderRadius: '50%', background: 'var(--signal-wash)', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }}
            />
          )}
          {/* Inner glow */}
          <div style={{ position: 'absolute', width: 60, height: 60, borderRadius: '50%', background: 'var(--signal-wash)', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }} />
          {/* Spinner ring */}
          <motion.div
            animate={reduce ? undefined : { rotate: 360 }}
            transition={{ duration: 1.5, repeat: Infinity, ease: 'linear' }}
            style={{ width: 56, height: 56, borderRadius: '50%', border: '2px solid var(--border)', borderTop: '2px solid var(--signal)', flexShrink: 0 }}
          />
        </div>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '21px', fontWeight: 700, color: 'var(--ink)', letterSpacing: '-0.03em', marginBottom: 8 }}>
        ...
        <AnimatePresence mode="wait">
          <motion.p
            ...
            style={{ fontSize: '13.5px', color: 'var(--muted)', marginBottom: 0 }}
          >
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                background: selected ? 'var(--signal-wash)' : 'var(--surface)',
                border: `1.5px solid ${selected ? 'var(--signal)' : 'var(--border)'}`,
                boxShadow: selected
                  ? '0 0 0 1px oklch(0.505 0.19 30 / 0.28), var(--shadow-md)'
                  : 'var(--shadow-sm)',
            ...
            >
              ...
              <AnimatePresence>
                {selected && (
                  <motion.div
                    initial={reduce ? false : { scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={reduce ? {} : { scale: 0, opacity: 0 }}
                    transition={reduce ? {} : { type: 'spring', stiffness: 360, damping: 22 }}
                    style={{ position: 'absolute', top: 14, right: 14, width: 22, height: 22, borderRadius: '50%', background: 'var(--signal)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                  >
              ...
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '14.5px', fontWeight: 700, color: 'var(--ink)', marginBottom: 4, paddingRight: 34, letterSpacing: '-0.015em' }}>
                {p.title}
              </div>
              <div style={{ fontSize: '12.5px', color: 'var(--muted)', lineHeight: 1.58, marginBottom: 10 }}>
                {p.description}
              </div>
              <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: '11px', fontWeight: 500, padding: '3px 9px', borderRadius: 99, background: 'var(--bg-2)', color: 'var(--muted)', border: '1px solid var(--border)' }}>
                  {p.marketSize}
                </span>
              </div>
            </motion.button>
  ...
  return (
    <div>
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                background: on ? 'var(--signal-wash)' : 'var(--surface)',
                border: `1.5px solid ${on ? 'var(--signal)' : 'var(--border)'}`,
                opacity: disabled ? 0.38 : 1,
                boxShadow: on ? '0 0 0 1px oklch(0.505 0.19 30 / 0.24), var(--shadow-md)' : 'var(--shadow-sm)',
                transition: 'background 0.18s, border-color 0.18s, opacity 0.18s, box-shadow 0.18s',
                position: 'relative', overflow: 'hidden',
              }}
            >
              {on && (
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: 'linear-gradient(90deg, transparent, var(--signal), transparent)', opacity: 0.55 }} />
              )}
              <span style={{ fontSize: '20px' }}>{c.icon}</span>
              <span style={{ fontSize: '13.5px', fontWeight: 600, color: 'var(--ink)', flex: 1 }}>{c.value}</span>
              <AnimatePresence>
                {on && (
                  <motion.div
                    initial={reduce ? false : { scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={reduce ? {} : { scale: 0, opacity: 0 }}
                    transition={reduce ? {} : { type: 'spring', stiffness: 360, damping: 22 }}
                    style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--signal)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
                  >
      ...
      <div style={{
        ...
        color: selected.length === 3 ? '#b45309' : 'var(--muted)',
      }}>
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                cursor: 'pointer', fontFamily: 'inherit', position: 'relative', overflow: 'hidden',
                background: sel ? `${ac}1f` : 'var(--surface)',
                border: `1.5px solid ${sel ? ac : 'var(--border)'}`,
                boxShadow: sel ? `0 0 0 1px ${ac}30, var(--shadow-md)` : 'var(--shadow-sm)',
                transition: 'background 0.2s, border-color 0.2s, box-shadow 0.2s',
              }}
            >
              {/* Top accent line when selected */}
              ...
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 800, color: 'var(--ink)', marginBottom: 3, letterSpacing: '-0.02em' }}>{o.label}</div>
              <div style={{ fontSize: '12.5px', color: 'var(--muted)', marginBottom: 16 }}>{o.sub}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                {o.examples.map((ex) => (
                  <div key={ex} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '11.5px', color: 'var(--ink-soft)' }}>
...
function LoadingIdeasStep({ loadingStep }: { loadingStep: number }) {
  ...
  return (
    <div style={{ textAlign: 'center', padding: '52px 0' }}>
      ...
      <p style={{ fontSize: '13.5px', color: 'var(--muted)', marginBottom: 36 }}>
          ...
          return (
            ...
            >
              <div style={{
                ...
                background: isDone ? 'var(--ok)' : isActive ? 'var(--signal-wash)' : 'var(--bg-2)',
                border: `1.5px solid ${isDone ? 'var(--ok)' : isActive ? 'var(--signal)' : 'var(--border)'}`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
                {isDone && <Check style={{ width: 11, height: 11, color: 'white' }} strokeWidth={3} />}
                {isActive && !reduce && (
                  <motion.div
                ...
                {isActive && reduce && <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--signal)' }} />}
              </div>
              <span style={{ fontSize: '13px', lineHeight: 1.4, fontWeight: isActive ? 500 : 400, color: isDone ? 'var(--muted)' : isActive ? 'var(--ink)' : 'var(--muted)' }}>
                {msg}
              </span>
            </motion.div>
          )
  ...
  return (
    <div>
      <div style={{ textAlign: 'center', marginBottom: 24 }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(22px, 3vw, 30px)', fontWeight: 800, letterSpacing: '-0.03em', color: 'var(--ink)', marginBottom: 6 }}>Tes opportunités SaaS</h2>
        <p style={{ fontSize: '13.5px', color: 'var(--muted)' }}>Choisis l'idée pour laquelle tu veux un blueprint complet.</p>
      </div>
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                cursor: 'pointer', fontFamily: 'inherit', position: 'relative', overflow: 'hidden',
                background: sel ? `${ac}1c` : 'var(--surface)',
                border: `1.5px solid ${sel ? ac : 'var(--border)'}`,
                boxShadow: sel ? `0 0 0 1px ${ac}30, var(--shadow-md)` : 'var(--shadow-sm)',
                transition: 'background 0.18s, border-color 0.18s, box-shadow 0.18s',
              }}
            >
              ...
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '14.5px', fontWeight: 700, color: 'var(--ink)', marginBottom: 3, paddingRight: sel ? 28 : 0, marginTop: 5, letterSpacing: '-0.015em' }}>
                {idea.name}
              </div>
              <div style={{ fontSize: '11.5px', color: 'var(--muted)', lineHeight: 1.5, marginBottom: 10, minHeight: 30 }}>
                {idea.tagline}
              </div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 9px', borderRadius: 99, background: 'var(--ok-wash)', border: '1px solid oklch(0.52 0.135 152 / 0.22)', marginBottom: 9 }}>
                <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--ok)' }}>{idea.mrrPotential}</span>
                <span style={{ fontSize: '9.5px', color: 'var(--ok)' }}>MRR</span>
              </div>
              <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: '10px', fontWeight: 600, padding: '3px 7px', borderRadius: 6, background: 'var(--bg-2)', color: 'var(--ink-soft)' }}>{idea.techComplexity}</span>
                <span style={{ fontSize: '10px', fontWeight: 600, padding: '3px 7px', borderRadius: 6, background: 'var(--bg-2)', color: 'var(--ink-soft)' }}>{idea.timeToMvp}</span>
              </div>
            </motion.button>
          )
        })}
...
const PLANS = [
  ...
  { id: 'month', price: '€23', period: '/month', name: 'Monthly', tagline: 'For serious builders', accent: '#c8442a', badge: 'MOST POPULAR', save: 'Save 19% vs weekly' },
  { id: 'year', price: '€75', period: '/year', name: 'Annual', tagline: 'Best long-term value', accent: '#2e7d50', badge: 'BEST VALUE', save: 'Save 73% vs monthly' },
]
  ...
  return (
    <div>
      <motion.div {...rise(0)} style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(22px, 3vw, 30px)', fontWeight: 800, letterSpacing: '-0.03em', color: 'var(--ink)', marginBottom: 6 }}>
          Unlock your <span style={{ color: 'var(--signal)' }}>{ideaName || 'SaaS'}</span> blueprint
        </h2>
        <p style={{ fontSize: '13.5px', color: 'var(--muted)' }}>Cancel anytime. Instant access to your full plan.</p>
      </motion.div>
          ...
          return (
            <motion.div key={p.id}
              ...
              style={{
                ...
                background: popular ? 'var(--signal-wash)' : 'var(--surface)',
                border: `1.5px solid ${popular ? 'var(--signal)' : 'var(--border)'}`,
              ...
              {p.badge && (
                <div style={{ position: 'absolute', top: -10, left: '50%', transform: 'translateX(-50%)', padding: '3px 12px', borderRadius: 99, fontSize: '9.5px', fontWeight: 800, letterSpacing: '0.05em', whiteSpace: 'nowrap',
                  background: p.accent, color: '#fff', boxShadow: `0 4px 14px ${p.accent}66` }}>{p.badge}</div>
              )}
              {popular ? (
                <div style={{ borderRadius: 18, padding: '20px 16px 18px', position: 'relative', overflow: 'hidden',
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 2, marginBottom: 4 }}>
                    <span style={{ fontSize: '28px', fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.03em' }}>{p.price}</span>
                    <span style={{ fontSize: '12px', color: 'var(--muted)' }}>{p.period}</span>
                  </div>
                  <div style={{ fontSize: '11px', color: p.save ? 'var(--ok)' : 'var(--muted)', marginBottom: 14, minHeight: 14 }}>{p.save || p.tagline}</div>
                  <motion.button whileTap={reduce ? undefined : { scale: 0.96 }} onClick={() => pick(p.id)} disabled={!!processing}
                    style={{ width: '100%', padding: '11px', borderRadius: 12, fontSize: '13px', fontWeight: 700, cursor: processing ? 'wait' : 'pointer', border: 'none', fontFamily: 'inherit',
                      background: 'var(--signal)', color: '#fff', boxShadow: 'var(--shadow-sm)',
                ...
                <>
                  ...
                  <div style={{ fontSize: '11px', color: p.save ? 'var(--ok)' : 'var(--muted)', marginBottom: 14, minHeight: 14 }}>{p.save || p.tagline}</div>
                  <motion.button whileTap={reduce ? undefined : { scale: 0.96 }} onClick={() => pick(p.id)} disabled={!!processing}
                    style={{ width: '100%', padding: '11px', borderRadius: 12, fontSize: '13px', fontWeight: 700, cursor: processing ? 'wait' : 'pointer', border: '1px solid var(--border-strong)', fontFamily: 'inherit',
      ...
      <motion.div {...rise(0.3)} style={{ borderRadius: 14, padding: '14px 18px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)', marginBottom: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px 16px' }}>
          {PLAN_FEATURES.map((f) => (
            <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '12px', color: 'var(--ink-soft)' }}>
              <div style={{ width: 16, height: 16, borderRadius: '50%', background: 'var(--ok-wash)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Check style={{ width: 9, height: 9, color: 'var(--ok)' }} strokeWidth={3} />
              </div>{f}
            </div>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 14 }}>
        <button onClick={onBack} style={{ fontSize: '12.5px', color: 'var(--muted)', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
    ...
    <div>
      <div style={{ textAlign: 'center', marginBottom: 26 }}>
        <motion.div initial={reduce ? false : { scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={reduce ? {} : { duration: 0.4 }}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 14px', borderRadius: 99, marginBottom: 14, background: 'var(--ok-wash)', border: '1px solid oklch(0.52 0.135 152 / 0.28)' }}>
          <Sparkles style={{ width: 12, height: 12, color: 'var(--ok)' }} />
          <span style={{ fontSize: '11px', color: 'var(--ok)', fontWeight: 600, letterSpacing: '0.06em' }}>BLUEPRINT PRÊT</span>
        </motion.div>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(24px, 3.2vw, 32px)', fontWeight: 800, letterSpacing: '-0.03em', color: 'var(--ink)' }}>Ton blueprint SaaS est prêt</h2>
      </div>
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 18 }}>
        ...
        <div style={{ flex: '1 1 340px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <motion.div initial={reduce ? false : { opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={reduce ? {} : { duration: 0.4 }}
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '22px', fontWeight: 800, color: 'var(--ink)', marginBottom: 4 }}>{idea.name}</div>
            <div style={{ fontSize: '13px', color: 'var(--ink-soft)', marginBottom: 14 }}>{idea.tagline}</div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 99, background: 'var(--ok-wash)', border: '1px solid oklch(0.52 0.135 152 / 0.25)', marginBottom: 14 }}>
              <span style={{ fontSize: '14px', fontWeight: 800, color: 'var(--ok)' }}>{idea.mrrPotential}</span>
              <span style={{ fontSize: '11px', color: 'var(--ok)' }}>Potentiel MRR</span>
            </div>
            <div style={{ padding: '10px 14px', borderRadius: 10, background: 'var(--surface)', border: '1px solid var(--border)', fontSize: '12.5px', color: 'var(--ink-soft)', fontStyle: 'italic' }}>💡 {idea.why}</div>
          </motion.div>

          <div style={{ borderRadius: 18, padding: '20px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <Megaphone style={{ width: 16, height: 16, color: 'var(--signal)' }} />
              <span style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: 'var(--ink)' }}>Stratégie marketing</span>
            </div>
            <p style={{ fontSize: '13px', color: 'var(--ink-soft)', lineHeight: 1.6, marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginBottom: 8 }}>
              {channels.map(c => (
                <span key={c} style={{ fontSize: '11px', fontWeight: 600, padding: '5px 11px', borderRadius: 99, background: 'var(--signal-wash)', color: 'var(--signal-ink)', border: '1px solid oklch(0.505 0.19 30 / 0.22)' }}>{c}</span>
              ))}
              <span style={{ fontSize: '11px', fontWeight: 600, padding: '5px 11px', borderRadius: 99, background: 'var(--bg-2)', color: 'var(--ink-soft)' }}>{userData.marketType === 'b2b' ? 'B2B' : 'B2C'}</span>
              <span style={{ fontSize: '11px', fontWeight: 600, padding: '5px 11px', borderRadius: 99, background: 'var(--bg-2)', color: 'var(--ink-soft)' }}>Ads: {userData.adsBudget || '€0'}</span>
            </div>
          </div>
          ...
          <div style={{ borderRadius: 18, padding: '20px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: 'var(--ink)' }}>Plan de lancement 30 jours</span>
            </div>
            <div style={{ position: 'relative', paddingLeft: 22 }}>
              <div style={{ position: 'absolute', left: 6, top: 4, bottom: 4, width: 2, background: 'linear-gradient(180deg, var(--signal), var(--ok))' }} />
              {LAUNCH_PLAN.map((m, i) => (
                <motion.div key={i} initial={reduce ? false : { opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={reduce ? {} : { duration: 0.3, delay: 0.1 + i * 0.08 }}
                  ...
                  <div style={{ position: 'absolute', left: -22, top: 2, width: 12, height: 12, borderRadius: '50%', background: 'var(--signal)', border: '2px solid var(--surface)', boxShadow: '0 0 0 2px oklch(0.505 0.19 30 / 0.3)' }} />
                  <div style={{ fontSize: '10.5px', fontWeight: 700, color: 'var(--signal-ink)', letterSpacing: '0.04em', marginBottom: 2 }}>{m.day}</div>
                  <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ink)', marginBottom: 2 }}>{m.title}</div>
                  <div style={{ fontSize: '12px', color: 'var(--muted)', lineHeight: 1.5 }}>{m.desc}</div>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <div style={{ flex: '1 1 260px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ borderRadius: 18, padding: '20px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
              <Target style={{ width: 16, height: 16, color: 'var(--error)' }} />
              <span style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: 'var(--ink)' }}>Concurrents principaux</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
              {competitors.slice(0, 3).map((c, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '10px 12px', borderRadius: 10, background: 'var(--bg-2)', border: '1px solid var(--border)' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ink)' }}>{c}</div>
                    <div style={{ fontSize: '10.5px', color: 'var(--muted)' }}>Concurrent établi</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ borderRadius: 18, padding: '20px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              <TrendingUp style={{ width: 16, height: 16, color: 'var(--ok)' }} />
              <span style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: 'var(--ink)' }}>Projection revenus</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 90, marginBottom: 8 }}>
              {bars.map((h, i) => (
                <motion.div key={i} initial={reduce ? false : { height: 0 }} animate={{ height: `${h}%` }} transition={reduce ? {} : { duration: 0.6, delay: 0.2 + i * 0.08, ease: EASE }}
                  style={{ flex: 1, borderRadius: '4px 4px 0 0', background: i >= 4 ? 'var(--ok)' : 'var(--signal)' }} />
              ))}
            </div>
          ...
          <div style={{ borderRadius: 18, padding: '20px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: 'var(--ink)', marginBottom: 12 }}>Ressources</div>
            {['Guide de lancement Product Hunt', 'Modèles de cold email', 'Checklist tech stack MVP'].map((r) => (
              <a key={r} href="#" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', fontSize: '12.5px', color: 'var(--signal-ink)', textDecoration: 'none', borderBottom: '1px solid var(--border)' }}>
      ...
      <motion.button whileHover={reduce ? undefined : { scale: 1.01 }} whileTap={reduce ? undefined : { scale: 0.99 }} onClick={onDashboard}
        style={{ width: '100%', padding: '15px', borderRadius: 14, background: 'var(--signal)', boxShadow: 'var(--shadow-md)', color: 'white', fontWeight: 700, fontSize: '15px', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: 'inherit' }}>
...
function DashboardStep({ ideas, userData, onRestart }: { ideas: SaaSIdea[]; userData: UserData; onRestart: () => void }) {
  ...
  const DASH_TABS: { id: DashTab; label: string; icon: string; accent: string }[] = [
    { id: 'overview', label: 'Vue d\'ensemble', icon: '🏠', accent: '#c8442a' },
  ...
  const dashPlans = [
    { id: 'week', name: 'Hebdo', price: '7,90€', period: '/ sem', save: '', accent: '#0284c7', ctaBg: 'rgba(2,132,199,0.12)', ctaBorder: 'rgba(2,132,199,0.3)', ctaColor: '#0369a1', border: 'rgba(2,132,199,0.25)', bg: 'rgba(2,132,199,0.05)', featured: false },
    { id: 'month', name: 'Mensuel', price: '23€', period: '/ mois', save: 'Économise 19 %', accent: 'var(--signal)', ctaBg: 'var(--signal)', ctaBorder: 'transparent', ctaColor: 'white', border: 'var(--signal)', bg: 'var(--signal-wash)', featured: true },
    { id: 'year', name: 'Annuel', price: '75€', period: '/ an', save: 'Économise 73 %', accent: 'var(--ok)', ctaBg: 'var(--ok-wash)', ctaBorder: 'oklch(0.52 0.135 152 / 0.3)', ctaColor: 'var(--ok)', border: 'oklch(0.52 0.135 152 / 0.25)', bg: 'var(--ok-wash)', featured: false },
  ]
        ...
        return (
          <button key={t.id} onClick={() => setTab(t.id)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 10, whiteSpace: 'nowrap', cursor: 'pointer', fontFamily: 'inherit', fontSize: '12.5px', fontWeight: active ? 700 : 500, transition: 'all 0.15s', flexShrink: 0,
              background: active ? `${t.accent}22` : 'var(--surface)',
              border: `1.5px solid ${active ? t.accent + '66' : 'var(--border)'}`,
              color: active ? t.accent : 'var(--ink-soft)' }}>
            <span style={{ fontSize: '14px' }}>{t.icon}</span>
            {t.label}
            {isConnected && <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--ok)', flexShrink: 0 }} />}
          </button>
        )
      })}
    </div>
  ...
  const OverviewContent = (
    <motion.div key="overview" initial={reduce ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={reduce ? {} : { duration: 0.25 }}>
      {/* Project banner */}
      <div style={{ borderRadius: 18, padding: '22px 24px', marginBottom: 16, position: 'relative', overflow: 'hidden', background: 'var(--signal-wash)', border: '1px solid oklch(0.505 0.19 30 / 0.22)' }}>
        <div style={{ position: 'absolute', top: -40, right: -20, width: 160, height: 160, borderRadius: '50%', background: 'radial-gradient(circle,oklch(0.505 0.19 30 / 0.12),transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ fontSize: '10.5px', fontWeight: 700, color: 'var(--signal-ink)', letterSpacing: '0.07em', marginBottom: 6 }}>TON ESPACE DE TRAVAIL</div>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(20px,3vw,28px)', fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.02em', marginBottom: 4 }}>{idea.name}</div>
        <div style={{ fontSize: '13px', color: 'var(--ink-soft)' }}>{idea.tagline}</div>
        <div style={{ display: 'flex', gap: 10, marginTop: 14, flexWrap: 'wrap' }}>
          {[{ label: 'MRR cible', value: idea.mrrPotential, color: 'var(--ok)' }, { label: 'MVP estimé', value: idea.timeToMvp, color: 'var(--signal-ink)' }, { label: 'Marché', value: userData.marketType === 'b2b' ? 'B2B' : 'B2C', color: '#0369a1' }, { label: 'Budget pub', value: userData.adsBudget || '—', color: '#b45309' }].map(s => (
            <div key={s.label} style={{ borderRadius: 10, padding: '8px 12px', background: 'var(--surface)', border: '1px solid var(--border)' }}>
              <div style={{ fontSize: '13.5px', fontWeight: 800, color: s.color }}>{s.value}</div>
              <div style={{ fontSize: '10px', color: 'var(--muted)', marginTop: 1 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ borderRadius: 18, padding: '18px 20px', marginBottom: 16, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: 'var(--ink)', marginBottom: 4 }}>Réseaux sociaux</div>
        <div style={{ fontSize: '12px', color: 'var(--muted)', marginBottom: 14 }}>Connecte tes comptes pour débloquer ta stratégie sur mesure dans chaque onglet.</div>
            ...
            return (
              <button key={p.id} onClick={() => setTab(p.id as DashTab)}
                style={{ borderRadius: 12, padding: '12px 8px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s', position: 'relative',
                  background: on ? `${p.accent}18` : 'var(--bg-2)', border: `1.5px solid ${on ? p.accent + '55' : 'var(--border)'}` }}>
                {on && <span style={{ position: 'absolute', top: 7, right: 7, width: 7, height: 7, borderRadius: '50%', background: 'var(--ok)' }} />}
                <span style={{ fontSize: '20px' }}>{p.icon}</span>
                <span style={{ fontSize: '10px', fontWeight: 600, color: on ? p.accent : 'var(--ink-soft)' }}>{p.label}</span>
                <span style={{ fontSize: '9.5px', color: on ? 'var(--ok)' : 'var(--muted)' }}>{on ? 'Connecté' : 'Voir →'}</span>
              </button>
            )
          })}
        </div>
      ...
      {/* Payment plans */}
      <div style={{ borderRadius: 18, padding: '20px', marginBottom: 16, background: 'var(--surface)', border: '1px solid oklch(0.505 0.19 30 / 0.22)', boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ textAlign: 'center', marginBottom: 16 }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: '15px', fontWeight: 700, color: 'var(--ink)', marginBottom: 3 }}>Débloque ton blueprint complet ✦</div>
          <div style={{ fontSize: '11.5px', color: 'var(--muted)' }}>Accès instantané · Annulation libre · Sans frais cachés</div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
          {dashPlans.map(p => (
            <div key={p.id} style={{ borderRadius: 14, padding: '16px 12px', background: p.bg, border: `1.5px solid ${p.border}`, position: 'relative', transform: p.featured ? 'scale(1.04)' : 'scale(1)', transition: 'transform 0.15s' }}>
              {p.featured && <div style={{ position: 'absolute', top: -10, left: '50%', transform: 'translateX(-50%)', background: 'var(--signal)', color: 'white', fontSize: '9.5px', fontWeight: 700, padding: '3px 10px', borderRadius: 99, whiteSpace: 'nowrap', boxShadow: 'var(--shadow-sm)' }}>✦ Populaire</div>}
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 3, marginBottom: p.save ? 2 : 10 }}>
                <span style={{ fontSize: '24px', fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.04em', lineHeight: 1 }}>{p.price}</span>
                <span style={{ fontSize: '10px', color: 'var(--muted)' }}>{p.period}</span>
              </div>
              {p.save && <div style={{ fontSize: '9.5px', color: 'var(--ok)', fontWeight: 600, marginBottom: 9 }}>{p.save}</div>}
              <button style={{ width: '100%', padding: '9px', borderRadius: 10, fontSize: '11.5px', fontWeight: 600, cursor: 'pointer', border: `1px solid ${p.ctaBorder}`, background: p.ctaBg, color: p.ctaColor, fontFamily: 'inherit', boxShadow: p.featured ? 'var(--shadow-sm)' : 'none' }}>
                Choisir →
              </button>
            </div>
          ))}
      ...
      <div style={{ borderRadius: 18, padding: '18px 20px', marginBottom: 16, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <span style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: 'var(--ink)' }}>Plan d&apos;action — semaine 1</span>
          <span style={{ fontSize: '12px', fontWeight: 700, color: doneCount === WEEK_TASKS.length ? 'var(--ok)' : 'var(--muted)' }}>{doneCount} / {WEEK_TASKS.length}</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          {WEEK_TASKS.map((t, i) => {
            const done = tasks[i]
            return (
              <button key={i} onClick={() => setTasks(p => p.map((v, j) => j === i ? !v : v))}
                style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '10px 13px', borderRadius: 10, cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit',
                  background: done ? 'var(--ok-wash)' : 'var(--bg-2)', border: `1px solid ${done ? 'oklch(0.52 0.135 152 / 0.22)' : 'var(--border)'}`, transition: 'all 0.15s' }}>
                <div style={{ width: 19, height: 19, borderRadius: 5, flexShrink: 0, background: done ? 'var(--ok)' : 'transparent', border: `1.5px solid ${done ? 'var(--ok)' : 'var(--border-strong)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {done && <Check style={{ width: 11, height: 11, color: 'white' }} strokeWidth={3} />}
                </div>
                <span style={{ fontSize: '12.5px', color: done ? 'var(--muted)' : 'var(--ink)', textDecoration: done ? 'line-through' : 'none' }}>{t}</span>
              </button>
            )
          })}
        </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
        <button style={{ flex: 1, padding: '12px', borderRadius: 13, fontSize: '13px', fontWeight: 600, cursor: 'pointer', border: 'none', fontFamily: 'inherit', background: 'var(--signal)', color: '#fff', boxShadow: 'var(--shadow-sm)' }}>Partager mon blueprint</button>
        <button style={{ flex: 1, padding: '12px', borderRadius: 13, fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', background: 'var(--surface)', color: 'var(--ink)', border: '1px solid var(--border-strong)' }}>Télécharger (PDF)</button>
      </div>
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <button onClick={onRestart} style={{ fontSize: '11.5px', color: 'var(--muted)', background: 'none', border: 'none', cursor: 'pointer' }}>Démarrer un nouveau projet</button>
      </div>
    </motion.div>
  )
    ...
    <motion.div key={activeCfg.id} initial={reduce ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={reduce ? {} : { duration: 0.25 }}>
      {connected[activeCfg.id] ? (
        ...
        <div>
          ...
          <div style={{ borderRadius: 18, padding: '18px 20px', marginBottom: 14, background: activeCfg.bg, border: `1.5px solid ${activeCfg.accent}44` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              ...
              <div>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: '17px', fontWeight: 800, color: 'var(--ink)' }}>{activeCfg.label}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 2 }}>
                  <span style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--ok)', display: 'inline-block' }} />
                  <span style={{ fontSize: '11.5px', color: 'var(--ok)', fontWeight: 600 }}>Compte connecté</span>
                </div>
              </div>
              <button onClick={() => setConnected(p => ({ ...p, [activeCfg.id]: false }))}
                style={{ marginLeft: 'auto', padding: '6px 12px', borderRadius: 8, fontSize: '11px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', background: 'var(--surface)', color: 'var(--ink-soft)', border: '1px solid var(--border-strong)' }}>
                Déconnecter
              </button>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 14 }}>
            {activeCfg.stats.map(s => (
              <div key={s.label} style={{ borderRadius: 14, padding: '14px 12px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
                <div style={{ fontSize: '17px', fontWeight: 800, color: activeCfg.accent, lineHeight: 1.1, marginBottom: 4 }}>{s.value}</div>
                <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--ink)', marginBottom: 2 }}>{s.label}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
            <div style={{ borderRadius: 16, padding: '16px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: 10 }}>Stratégie de contenu</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                {activeCfg.strategy.map((s, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--ink-soft)', lineHeight: 1.4 }}>{s}</span>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ borderRadius: 16, padding: '16px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: 10 }}>Formats prioritaires</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {activeCfg.formats.map(f => (
                  <div key={f.name} style={{ padding: '10px 12px', borderRadius: 10, background: `${activeCfg.accent}11`, border: `1px solid ${activeCfg.accent}33` }}>
                    ...
                    <div style={{ fontSize: '10.5px', color: 'var(--muted)' }}>{f.tip}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Posting schedule */}
          <div style={{ borderRadius: 16, padding: '16px 18px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: 12 }}>Calendrier de publication</div>
              ...
              {activeCfg.schedule.map(slot => (
                <div key={slot.day} style={{ borderRadius: 12, padding: '12px 10px', textAlign: 'center', background: `${activeCfg.accent}0F`, border: `1px solid ${activeCfg.accent}33` }}>
                  ...
                  {slot.times.split(' · ').map(t => (
                    <div key={t} style={{ fontSize: '11px', color: 'var(--ink-soft)', background: 'var(--bg-2)', borderRadius: 6, padding: '3px 6px', marginBottom: 4 }}>{t}</div>
                  ))}
                </div>
              ))}
            </div>
        ...
        <div>
          ...
          <div style={{ borderRadius: 18, padding: '18px 20px', marginBottom: 14, background: 'var(--surface)', border: `1.5px solid ${activeCfg.accent}22`, boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: '17px', fontWeight: 800, color: 'var(--ink)' }}>{activeCfg.label}</div>
                <div style={{ fontSize: '11.5px', color: 'var(--muted)', marginTop: 2 }}>Non connecté</div>
              </div>
            </div>
          </div>

          {/* Blurred stats preview */}
          <div style={{ borderRadius: 16, padding: '16px', marginBottom: 14, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)', position: 'relative', overflow: 'hidden' }}>
            <div style={{ filter: 'blur(6px)', userSelect: 'none', pointerEvents: 'none' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
                {activeCfg.stats.map(s => (
                  <div key={s.label} style={{ borderRadius: 14, padding: '14px 12px', background: 'var(--bg-2)', border: '1px solid var(--border)' }}>
                    <div style={{ fontSize: '17px', fontWeight: 800, color: activeCfg.accent, marginBottom: 4 }}>{s.value}</div>
                    <div style={{ fontSize: '11px', color: 'var(--ink-soft)', marginBottom: 2 }}>{s.label}</div>
                    <div style={{ fontSize: '10px', color: 'var(--muted)' }}>{s.sub}</div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              ...
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '14px', fontWeight: 700, color: 'var(--ink)' }}>Statistiques personnalisées</div>
              <div style={{ fontSize: '12px', color: 'var(--muted)' }}>Connecte ton compte pour voir tes vraies données</div>
            </div>
          </div>

          {/* Unlocks list */}
          <div style={{ borderRadius: 16, padding: '16px 18px', marginBottom: 18, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: 10 }}>Ce que tu débloqueras</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              {activeCfg.unlocks.map((u, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '9px 12px', borderRadius: 10, background: `${activeCfg.accent}0C`, border: `1px solid ${activeCfg.accent}22` }}>
                  ...
                  <span style={{ fontSize: '12.5px', color: 'var(--ink-soft)' }}>{u}</span>
                </div>
              ))}
            </div>
          </div>
  ...
  const MetaAdsContent = (
    <motion.div key="metaads" initial={reduce ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={reduce ? {} : { duration: 0.25 }}>
      {/* Header */}
      <div style={{ borderRadius: 18, padding: '18px 20px', marginBottom: 14, background: 'rgba(24,119,242,0.08)', border: '1.5px solid rgba(24,119,242,0.35)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
          ...
          <div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '17px', fontWeight: 800, color: 'var(--ink)' }}>Meta Ads</div>
            <div style={{ fontSize: '11.5px', color: 'var(--muted)', marginTop: 2 }}>Facebook & Instagram Ads</div>
          </div>
          <div style={{ marginLeft: 'auto', textAlign: 'right' }}>
            <div style={{ fontSize: '12px', color: 'var(--muted)' }}>Budget pub mensuel</div>
            <div style={{ fontSize: '18px', fontWeight: 800, color: '#0369a1' }}>{userData.adsBudget || '—'}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 14 }}>
        {[
          { label: 'CPM estimé', value: '3,80 €', sub: 'coût pour 1 000 vues', color: '#0369a1' },
          { label: 'CPC moyen', value: '0,72 €', sub: 'coût par clic', color: 'var(--signal-ink)' },
          { label: 'ROAS cible', value: '3,2×', sub: 'retour sur dépense pub', color: 'var(--ok)' },
          { label: 'CTR moyen', value: '2,4 %', sub: 'taux de clic', color: '#b45309' },
        ].map(k => (
          <div key={k.label} style={{ borderRadius: 14, padding: '14px 12px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ fontSize: '19px', fontWeight: 800, color: k.color, lineHeight: 1, marginBottom: 4 }}>{k.value}</div>
            <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--ink)', marginBottom: 2 }}>{k.label}</div>
      ...
      <div style={{ borderRadius: 16, padding: '16px 18px', marginBottom: 14, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: 10 }}>Formats recommandés — {isB2B ? 'B2B' : 'B2C'}</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {(isB2B ? [
            ...
            <div key={f.name} style={{ padding: '11px 13px', borderRadius: 11, background: 'rgba(24,119,242,0.08)', border: '1px solid rgba(24,119,242,0.25)' }}>
              <div style={{ fontSize: '12px', fontWeight: 700, color: '#0369a1', marginBottom: 3 }}>{f.name}</div>
              <div style={{ fontSize: '10.5px', color: 'var(--muted)', marginBottom: 3 }}>{f.tip}</div>
              <div style={{ fontSize: '10px', color: 'var(--ok)', fontWeight: 600 }}>{f.roi}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Budget allocation */}
      <div style={{ borderRadius: 16, padding: '16px 18px', marginBottom: 14, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: 14 }}>Répartition du budget conseillée</div>
        {[
          { label: 'Notoriété (Awareness)', pct: 30, color: '#c8442a' },
          { label: 'Conversion', pct: 50, color: '#1877F2' },
          { label: 'Retargeting', pct: 20, color: '#2e7d50' },
        ].map(b => (
          <div key={b.label} style={{ marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
              <span style={{ fontSize: '12px', color: 'var(--ink-soft)' }}>{b.label}</span>
      ...
      {/* Mock campaigns */}
      <div style={{ borderRadius: 16, padding: '16px 18px', background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: '13px', fontWeight: 700, color: 'var(--ink)', marginBottom: 12 }}>Campagnes à lancer</div>
        {[
          { name: '🎯 Campagne Notoriété', status: 'Prête à configurer', spend: '—', roas: '—', color: '#c8442a' },
          { name: '💰 Campagne Conversion', status: 'Prête à configurer', spend: '—', roas: '—', color: '#1877F2' },
          { name: '🔁 Campagne Retargeting', status: 'Nécessite le Pixel', spend: '—', roas: '—', color: '#2e7d50' },
        ].map(c => (
          <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 11, marginBottom: 8, background: 'var(--bg-2)', border: '1px solid var(--border)' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ink)', marginBottom: 2 }}>{c.name}</div>
              <div style={{ fontSize: '10.5px', color: 'var(--muted)' }}>{c.status}</div>
            </div>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 18 }}>
        <div style={{ fontSize: '10.5px', fontWeight: 700, color: 'var(--signal-ink)', letterSpacing: '0.07em', marginBottom: 4 }}>TON DASHBOARD</div>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(20px,3vw,26px)', fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.02em' }}>{idea.name}</div>
      </div>
...
function AuthGateModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  const inputStyle: React.CSSProperties = { width: '100%', padding: '11px 14px', borderRadius: 10, background: 'var(--bg)', border: '1px solid var(--border-strong)', color: 'var(--ink)', fontSize: '14px', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }

  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'oklch(0.205 0.012 45 / 0.45)', backdropFilter: 'blur(6px)' }}>
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }} transition={{ duration: 0.2 }}
        style={{ width: '100%', maxWidth: 420, borderRadius: 20, padding: 32, background: 'var(--surface)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-lg)', position: 'relative' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', padding: 4 }}>
        ...
        {done ? (
          <div style={{ textAlign: 'center', padding: '16px 0' }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '18px', fontWeight: 800, color: 'var(--ink)', marginBottom: 8 }}>Vérifie ta boîte mail</h2>
            <p style={{ fontSize: '13px', color: 'var(--ink-soft)', lineHeight: 1.6 }}>
              Clique sur le lien envoyé à <strong style={{ color: 'var(--ink)' }}>{email}</strong> pour activer ton compte et accéder à ton espace.
            </p>
          </div>
        ) : (
          <>
            <div style={{ marginBottom: 4, fontSize: '11px', fontWeight: 700, color: 'var(--signal-ink)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Sauvegarde tes résultats</div>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 800, color: 'var(--ink)', marginBottom: 4, letterSpacing: '-0.02em' }}>
              {mode === 'signup' ? 'Crée ton compte gratuit' : 'Connecte-toi'}
            </h2>
            <p style={{ fontSize: '12.5px', color: 'var(--muted)', marginBottom: 20 }}>
              {mode === 'signup' ? 'Pour sauvegarder ton blueprint et accéder à ton espace.' : 'Retrouve ton espace de travail.'}
            </p>

            <button onClick={handleGoogle} style={{ width: '100%', padding: '11px', borderRadius: 12, background: 'var(--surface)', border: '1px solid var(--border-strong)', color: 'var(--ink)', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
              <span style={{ fontSize: '11px', color: 'var(--muted)', fontWeight: 600 }}>ou</span>
              <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
            </div>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {error && <div style={{ padding: '9px 12px', borderRadius: 8, background: 'oklch(0.55 0.20 25 / 0.08)', border: '1px solid oklch(0.55 0.20 25 / 0.22)', color: 'var(--error)', fontSize: '12.5px' }}>{error}</div>}
              <button type="submit" disabled={loading} style={{ padding: '12px', borderRadius: 12, background: 'var(--signal)', color: 'white', fontSize: '14px', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', border: 'none', fontFamily: 'inherit', opacity: loading ? 0.7 : 1, boxShadow: 'var(--shadow-sm)' }}>
                {loading ? '…' : mode === 'signup' ? 'Créer mon compte →' : 'Se connecter →'}
              </button>
            </form>

            <p style={{ textAlign: 'center', marginTop: 14, fontSize: '12.5px', color: 'var(--muted)' }}>
              {mode === 'signup' ? 'Déjà un compte ?' : 'Pas encore de compte ?'}{' '}
              <button onClick={() => setMode(mode === 'signup' ? 'login' : 'signup')} style={{ color: 'var(--signal-ink)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '12.5px', fontFamily: 'inherit' }}>
...
export function BuilderClient() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      {/* Ambient background — warm dotted field + soft vermilion glow */}
      <div className="dot-field" style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0, opacity: 0.5, maskImage: 'radial-gradient(ellipse 90% 60% at 50% 0%, #000 30%, transparent 80%)', WebkitMaskImage: 'radial-gradient(ellipse 90% 60% at 50% 0%, #000 30%, transparent 80%)' }} />
      <div style={{ position: 'fixed', top: '-22%', left: '50%', transform: 'translateX(-50%)', width: '760px', height: '540px', pointerEvents: 'none', zIndex: 0, background: 'radial-gradient(circle, oklch(0.505 0.19 30 / 0.08) 0%, oklch(0.505 0.19 30 / 0.03) 38%, transparent 72%)' }} />
      <div style={{ position: 'fixed', top: '8%', left: '12%', width: '420px', height: '420px', pointerEvents: 'none', zIndex: 0, background: 'radial-gradient(circle, oklch(0.505 0.19 30 / 0.04) 0%, transparent 70%)' }} />

      {/* Top nav */}
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', background: 'oklch(0.985 0.004 50 / 0.85)', backdropFilter: 'blur(16px)', borderBottom: '1px solid var(--border)' }}>
        <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
          <img src="/logo.png" alt="SaaSGenrt" style={{ width: 26, height: 26, objectFit: 'contain', flexShrink: 0 }} />
          <span style={{ fontSize: '14px', fontWeight: 600, color: 'var(--ink)' }}>SaaSGenrt</span>
        </a>
        {step >= 1 && step <= 12 && (
          <div style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 600, letterSpacing: '0.04em' }}>{step} / 12</div>
        )}
        <div style={{ width: 60 }} />
      </div>
